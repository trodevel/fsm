...
void init_fsm_5( fsm::Process * fsm )
{
    ...
    auto PLAYING_MESSAGE_2__T__ac1      = fsm->create_set_first_action_connector( PLAYING_MESSAGE_2__T, new fsm::SendSignal( "ScenRecordMessage", { fsm::ExpressionPtr( new fsm::ExpressionValue( fsm::Value( 1 ) ) ), fsm::ExpressionPtr( new fsm::ExpressionValue( fsm::Value( 15 ) ) ) } ));
    ...
    auto WAITING_ACTION__RecordFinished         = fsm->create_add_signal_handler( WAITING_ACTION, "RecordFinished" );
    auto WAITING_ACTION__RecordFinished__ac1    = fsm->create_set_first_action_connector( WAITING_ACTION__RecordFinished,
            new fsm::Task(
                    response,
                    fsm::ExpressionPtr( new fsm::ExpressionVariableName( "$1" ) ) ) );

    auto WAITING_ACTION__RecordFinished__ac2    = fsm->create_set_next_action_connector( WAITING_ACTION__RecordFinished__ac1,
    ...
    auto WAITING_ACTION__RecordFinished__ac3    = fsm->create_set_next_action_connector( WAITING_ACTION__RecordFinished__ac2,
    ...
    fsm->create_set_default_switch_action_connector( WAITING_ACTION__RecordFinished__ac3, new fsm::NextState( WAITING_ACTION ) );
    fsm->create_add_switch_action_connector( WAITING_ACTION__RecordFinished__ac3, new fsm::NextState( WAITING_ACTION ) );
    fsm->create_add_switch_action_connector( WAITING_ACTION__RecordFinished__ac3, new fsm::NextState( WAITING_ACTION ) );

    auto WAITING_ACTION__RecordFinished__ac4    = fsm->create_add_switch_action_connector( WAITING_ACTION__RecordFinished__ac3, new fsm::SendSignal( "ScenPlayMessage", { fsm::ExpressionPtr( new fsm::ExpressionVariable( action_message ) )} ));
    fsm->create_set_next_action_connector( WAITING_ACTION__RecordFinished__ac4, new fsm::NextState( PLAYING_MESSAGE_ACTION ) );

    auto WAITING_ACTION__RecordFailed           = fsm->create_add_signal_handler( WAITING_ACTION, "RecordFailed" );
    auto WAITING_ACTION__RecordFailed__ac1      = fsm->create_set_first_action_connector( WAITING_ACTION__RecordFailed,
            new fsm::SendSignal( "ScenExit",
            {
                    fsm::ExpressionPtr( new fsm::ExpressionVariable( E_DONE ) ),
                    fsm::ExpressionPtr( new fsm::ExpressionValue( fsm::Value( "timeout" ) ) )
            } ));
    fsm->set_next_action_connector( WAITING_ACTION__RecordFailed__ac1, WAITING_ACTION__Cancel__ac2 );
